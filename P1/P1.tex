...
\begin{enumerate}[label = (\arabic*)] 
	...
	\item If the keeping probability is high, then the model observes less noise, and the model capability is high. This means that the model might overfit. If the keeping probability is low, then the model observes more noise, and the model capability is low. This means that lower probability has a more regularizing effect, but our model might underfit. Figure \ref{fig:dropout_comparison} shows the result for different dropout probabilities for our fully connected network. We inserted a dropout layer between the third layer and the fourth layer. \textbf{Note that the} \verb|prob| \textbf{parameter here is the probability of dropping out a neuron instead of keeping it.} We observe that \verb|prob|=0.2 through 0.7 works effectively, while others perform worse. We note that \verb|prob|=0.1 performs the worst, which is expected as the regularization effect coming from dropout is low.
    \begin{figure}[!htbp]
        \centering
