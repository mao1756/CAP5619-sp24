        ...
        \State Initialize the temporary storage for $N_1,N_2,N_3$. We only update them after the end of a round to avoid shrinking the search space too fast.$(\bar{N}_1,\bar{N}_2,\bar{N}_3) = (N_1,N_2,N_3)$
         ...
         \For{rounds=$1,\cdots , r$}
            \For{sample=$1,\cdots ,s$}
                ...
                \State $(\bar{N}_1,\bar{N}_2,\bar{N}_3) = (\hat{n}_1,\hat{n}_2,\hat{n}_3)$
                \State $a$ = Accuracy for this sample
                \EndIf
            \EndFor
            \State $(N_1,N_2,N_3) = (\hat{N}_1,\hat{N}_2,\hat{N}_3)$
...
\begin{enumerate}[label = (\arabic*)] % Sub-questions styled as italic letters
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item The learning is too fast for  \textbf{Zero}, \textbf{One}, \textbf{Uniform} \textbf{Normal}, \textbf{Orthogonal}, \textbf{Uniform Xavier}. They do not generalize well compared to Normal Xavier.
    \end{enumerate}
	...
	\item Figure \ref{fig:fc_lr} shows the compassion between different learning rates for the fully connected network. We fixed the seeds of the random number generator to obtain the same initialization. The losses at the beginning are different because these are losses after one update.
    \begin{figure}[!htbp]
        \centering
...
\begin{enumerate}[label = (\arabic*)] 
	...
	\item If the keeping probability is high, then the model observes less noise, and the model capability is high. This means that the model might overfit. If the keeping probability is low, then the model observes more noise, and the model capability is low. This means that lower probability has a more regularizing effect, but our model might underfit. Figure \ref{fig:dropout_comparison} shows the result for different dropout probabilities for our fully connected network. We inserted a dropout layer between the third layer and the fourth layer. \textbf{Note that the} \verb|prob| \textbf{parameter here is the probability of dropping out a neuron instead of keeping it.} We observe that the models with lower \verb|prob| (i.e., 0.1 and 0.2) overfit the data, and the validation accuracy is low. For the rest of the parameters, they are effective with little variation. It might be surprising that the higher dropout probability does not lead to underfitting. It is probably because we already have so many neurons in the third layer (737 neurons), so 10\% of them is approximately 74 neurons, which is still a significant amount.
    \begin{figure}[!htbp]
        \centering
